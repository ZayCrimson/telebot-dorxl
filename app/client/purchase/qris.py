...
def show_qris_payment(
    ...
    # Jangan kirim QR sebagai ASCII terminal ke Telegram.
    # Telegram akan wrap font/line-height dan hasilnya jadi lonjong, bukan kotak.
    # Simpan QR sebagai PNG lalu bot_telegram.py akan mengirimnya sebagai foto.
    qr = qrcode.QRCode(
        version=None,
        error_correction=qrcode.constants.ERROR_CORRECT_M,
        box_size=int(os.getenv("QRIS_BOX_SIZE", "10")),
        border=int(os.getenv("QRIS_BORDER", "4")),
    )
    qr.add_data(qris_code)
    qr.make(fit=True)

    qr_dir = Path(os.getenv("QRIS_OUTPUT_DIR", "tmp_qris"))
    qr_dir.mkdir(parents=True, exist_ok=True)
    qr_path = qr_dir / f"qris_{int(time.time())}_{uuid.uuid4().hex[:8]}.png"
    img = qr.make_image(fill_color="black", back_color="white")
    img.save(qr_path)

    # Marker khusus dibaca bot Telegram. Jangan diubah kecuali bot-nya ikut diubah.
    print(f"QRIS_IMAGE_PATH:{qr_path.resolve()}")
